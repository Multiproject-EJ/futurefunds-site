...
async function initEditor() {
  ...
  const editPromptBtn = document.getElementById('editPromptList');
  ...
  const promptEditorBody = document.getElementById('promptEditorBody');
  ...
  const promptEditorNewBtn = document.getElementById('promptEditorNew');
  const promptEditorArchiveBtn = document.getElementById('promptEditorArchive');
  ...
  let promptEditorDraft = null;
  let promptEditorIsNew = false;
  ...
  const setPromptEditorDraft = (prompt, { isNew = false, skipSelectUpdate = false } = {}) => {
    const fallbackOrder = promptOptions.length ? promptOptions.length + 1 : 1;
    promptEditorDraft = prompt
      ? {
          ...prompt,
          sort_order: Number.isFinite(prompt.sort_order)
            ? prompt.sort_order
            : Number.parseInt(prompt.sort_order, 10) || fallbackOrder,
        }
      : {
          id: '',
          name: '',
          ...
          is_default: !promptOptions.length,
        };
    promptEditorIsNew = isNew;
    if (promptEditorName) promptEditorName.value = promptEditorDraft.name || '';
    if (promptEditorDescription) promptEditorDescription.value = promptEditorDraft.description || '';
    if (promptEditorBody) promptEditorBody.value = promptEditorDraft.prompt_text || '';
    if (promptEditorDefault) promptEditorDefault.checked = Boolean(promptEditorDraft.is_default);
    if (!skipSelectUpdate && promptEditorSelect) {
      if (promptEditorDraft.id && Array.from(promptEditorSelect.options).some((opt) => opt.value === promptEditorDraft.id)) {
        promptEditorSelect.value = promptEditorDraft.id;
      } else {
        promptEditorSelect.selectedIndex = -1;
      }
    }
    if (promptEditorArchiveBtn) {
      promptEditorArchiveBtn.disabled = !promptEditorDraft.id;
    }
  };

  const refreshPromptEditorOptions = () => {
    if (!promptEditorSelect) return;
    promptEditorSelect.innerHTML = '';
    if (!promptOptions.length) {
      const opt = document.createElement('option');
      opt.value = '';
      opt.textContent = 'No prompts saved yet';
      promptEditorSelect.appendChild(opt);
      promptEditorSelect.disabled = true;
      setPromptEditorDraft(null, { isNew: true, skipSelectUpdate: true });
      if (promptEditor && !promptEditor.hidden) {
        setPromptEditorStatus('No saved prompts yet. Draft one below and click save to store it.', 'info');
      }
      return;
    }
    promptEditorSelect.disabled = false;
    ...
    const currentId = promptEditorDraft?.id;
    if (currentId && promptOptions.some((opt) => opt.id === currentId)) {
      promptEditorSelect.value = currentId;
    } else {
      const fallbackId = selectedPrompt?.id || promptOptions[0].id;
      promptEditorSelect.value = fallbackId;
      if (!promptEditorIsNew) {
        const fallbackPrompt = promptOptions.find((opt) => opt.id === fallbackId);
        setPromptEditorDraft(fallbackPrompt, { isNew: false, skipSelectUpdate: true });
      }
    }
    if (promptEditorArchiveBtn) {
      promptEditorArchiveBtn.disabled = !promptEditorDraft?.id;
  ...
  const openPromptEditorPanel = async () => {
    ...
    refreshPromptEditorOptions();
    if (!promptEditorDraft && promptOptions.length) {
      const base = selectedPrompt || promptOptions[0];
      setPromptEditorDraft(base, { isNew: false, skipSelectUpdate: true });
    }
    promptEditor.hidden = false;
    setPromptEditorStatus('', 'info');
  };
  ...
  const startNewPromptDraft = () => {
    setPromptEditorDraft(null, { isNew: true, skipSelectUpdate: true });
    if (promptEditorSelect) {
      promptEditorSelect.selectedIndex = -1;
    }
    setPromptEditorStatus('Drafting a new prompt. Add a name and body, then save.', 'info');
  };

  const buildPromptPayloadFromDraft = () => {
    const name = (promptEditorName?.value || '').trim();
    const description = (promptEditorDescription?.value || '').trim();
    const body = (promptEditorBody?.value || '').trim();
    const isDefault = promptEditorDefault?.checked ?? false;
    if (!name) throw new Error('Prompt name is required.');
    if (!body) throw new Error('Prompt body cannot be empty.');
    let id = (promptEditorDraft?.id || '').trim();
    if (!id) {
      id = slugify(name);
    }
    if (!id) throw new Error('Unable to determine a prompt ID.');
    const fallbackOrder = promptOptions.length ? promptOptions.length + 1 : 1;
    const sortOrder = Number.isFinite(promptEditorDraft?.sort_order)
      ? promptEditorDraft.sort_order
      : Number.parseInt(promptEditorDraft?.sort_order, 10) || fallbackOrder;
    return {
      id,
      name,
      description,
  ...
  const savePromptDraft = async () => {
    let payload;
    try {
      payload = buildPromptPayloadFromDraft();
    } catch (err) {
      setPromptEditorStatus(err.message || 'Unable to save prompt.', 'error');
      return;
    }
    setPromptEditorStatus('Saving prompt…', 'info');
    try {
      const { error } = await supabase.from(PROMPT_TABLE).upsert([payload], { onConflict: 'id' });
      if (error) throw error;
      if (payload.is_default) {
        try {
          await supabase.from(PROMPT_TABLE).update({ is_default: false }).neq('id', payload.id);
        } catch (defaultError) {
          console.warn('Prompt default sync error', defaultError);
        }
      }
      desiredPromptId = payload.id;
      promptEditorDraft = { ...payload };
      promptEditorIsNew = false;
      await refreshPromptOptions();
      setSelectedPrompt(payload.id, { persist: true });
      refreshPromptEditorOptions();
      if (promptEditorSelect) {
        promptEditorSelect.value = payload.id;
      }
      setPromptEditorStatus('Prompt saved.', 'success');
      setTimeout(() => setPromptEditorStatus('', 'info'), 1500);
    } catch (error) {
  ...
  const archivePromptDraft = async () => {
    if (!promptEditorDraft?.id) {
      setPromptEditorStatus('Select a saved prompt before archiving.', 'error');
      return;
    }
    if (promptEditorIsNew) {
      setPromptEditorStatus('Save the prompt before archiving it.', 'error');
      return;
    }
    if (typeof window !== 'undefined' && !window.confirm('Archive this prompt template?')) {
      return;
    }
    setPromptEditorStatus('Archiving prompt…', 'info');
    try {
      const { error } = await supabase
        .from(PROMPT_TABLE)
        .update({ archived: true, is_default: false })
        .eq('id', promptEditorDraft.id);
      if (error) throw error;
      if (selectedPrompt?.id === promptEditorDraft.id) {
        selectedPrompt = null;
        desiredPromptId = null;
      }
      await refreshPromptOptions();
      refreshPromptEditorOptions();
      setPromptEditorDraft(null, { isNew: true, skipSelectUpdate: true });
      setPromptEditorStatus('Prompt archived.', 'success');
      setTimeout(() => setPromptEditorStatus('', 'info'), 1500);
    } catch (error) {
      console.error('Prompt archive error', error);
      setPromptEditorStatus(error.message || 'Unable to archive prompt.', 'error');
    }
  };
  ...
  const setSelectedPrompt = (id, { persist = true } = {}) => {
    ...
    updatePromptUI();
    if (promptEditor) {
      const draftTarget = next || null;
      if (promptEditor.hidden) {
        if (!promptEditorIsNew) {
          setPromptEditorDraft(draftTarget, { isNew: false, skipSelectUpdate: true });
        }
      } else if (!promptEditorIsNew) {
        setPromptEditorDraft(draftTarget, { isNew: false });
        refreshPromptEditorOptions();
      }
    }
...
')
  ...
  if (editPromptBtn) {
    editPromptBtn.addEventListener('click', async () => {
      if (promptEditor?.hidden ?? true) {
        await openPromptEditorPanel();
      } else {
  ...
  if (promptEditorSelect) {
    promptEditorSelect.addEventListener('change', () => {
      const value = promptEditorSelect.value;
      if (!value) {
        startNewPromptDraft();
        return;
      }
      const found = promptOptions.find((opt) => opt.id === value);
      if (found) {
        setPromptEditorDraft(found, { isNew: false, skipSelectUpdate: true });
        setPromptEditorStatus('', 'info');
      }
    });
  }
  ...
  if (promptEditorSaveBtn) {
    promptEditorSaveBtn.addEventListener('click', () => {
      savePromptDraft();
    });
  }
  ...
  if (promptEditorArchiveBtn) {
    promptEditorArchiveBtn.addEventListener('click', () => {
      archivePromptDraft();
    });
  }
